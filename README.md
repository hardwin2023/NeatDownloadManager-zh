...
本项目通过直接修改应用内的二进制 NIB 界面文件（`keyedobjects.nib`）中的字符串对象，实现界面汉化，无需修改可执行文件。
...
**总计 144 条 UI 字符串翻译，覆盖所有 13 个 NIB 界面文件。**

> **注意**：可执行二进制中硬编码的部分状态/错误信息（如 "Download In Progress"、"Application is Up To Date" 等）未汉化。这些文字主要通过程序代码动态设置，不在 NIB 文件中。
