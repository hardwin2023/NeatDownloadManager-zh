#!/usr/bin/env python3
"""
Neat Download Manager NIB 汉化脚本 (直接二进制 plist 修改)
直接修改 keyedobjects.nib 中的 $objects 字符串数组，绕过 ibtool
"""
...
def translate_nib(nib_path, output_path):
    """直接修改二进制 nib (plist) 中的字符串"""
...
def main():
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'work', 'translated_nibs')
    os.makedirs(out_dir, exist_ok=True)

    total = 0
    for name in sorted(os.listdir(app_res)):
        if not name.endswith('.nib'):
            continue

        src = os.path.join(app_res, name)
        dst = os.path.join(out_dir, name)

        if os.path.isdir(src):
            # Directory-format nib: translate keyedobjects.nib and keyedobjects-110000.nib
            os.makedirs(dst, exist_ok=True)
            count = 0
            for nib_file in ['keyedobjects.nib', 'keyedobjects-110000.nib']:
                nib_path = os.path.join(src, nib_file)
                if os.path.exists(nib_path):
                    c = translate_nib(nib_path, os.path.join(dst, nib_file))
                    count += c
            print(f"  {name}: {count} 处翻译 (目录格式)")
            total += count
        else:
            # Single-file nib
            count = translate_nib(src, dst)
            print(f"  {name}: {count} 处翻译 (单文件)")
            total += count
